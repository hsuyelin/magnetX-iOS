...
public let glt_iphoneX = UIScreen.instancesRespond(to:#selector(getter: UIScreen.main.currentMode)) ? __CGSizeEqualToSize(CGSize(width:1125, height:2436), (UIScreen.main.currentMode?.size)!) : false

public let glt_statusBarHeight = UIApplication.shared.statusBarFrame.size.height
public let glt_navHeight = 44.0
public let glt_navWithStatusBarHeight = glt_statusBarHeight + CGFloat(glt_navHeight)
